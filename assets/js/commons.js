...
export function timeAgo(time) {
    ...
    if (diffInYears > 0) {
        ...
        return {
            value: Math.max(1, diffInMinutes),
            unit:"time.minuteAgo"
        }
    }
...
export function removeLanguageDomainAndSub(route) {
    // 遍历语言数组
    for (const lang of allLang.LanguageList) {
        // 检查路由是否以当前语言的domain开头
        if (route.startsWith(lang.domain)) {
            // 如果是，去掉domain部分
            route = route.substring(lang.domain.length);
            break; // 找到匹配的domain后退出循环
        }
    }
    // 去除最后一个地址部分
    // 找到最后一个'/'的位置
    ...
    // 如果找到'/'，并且它不是路由的最后一个字符
    if (lastSlashIndex !== -1 && lastSlashIndex !== route.length - 1) {
        // 截取到这个位置
        route = route.slice(0, lastSlashIndex);
    }
    // 返回处理后的路由
    return route;
}
